...
__global__ void histogram_priv(const unsigned char *data, unsigned int *hist,
                               int n) {
    __shared__ unsigned int buf[BINS];

    // set bins 0
    for (int bin = threadIdx.x; bin < BINS; bin += blockDim.x) buf[bin] = 0;
    __syncthreads();

    // modify private bins
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for (; i < n; i += stride) atomicAdd(&buf[data[i]], 1u);
    __syncthreads();

    // add private bins to global hist
    for (int bin = threadIdx.x; bin < BINS; bin += blockDim.x) atomicAdd(&hist[bin], buf[bin]);
}
